<template>
    <div class="bg-zen min-h-screen py-12">
      <div class="container mx-auto px-4">
        ...
        <p v-motion="{ initial: { opacity: 0 }, enter: { opacity: 1, transition: { delay: 300 } } }" 
          ...
          Explorez notre sélection de bonsaïs robustes pour votre jardin ou votre terrasse
        </p>
  
        <div v-if="loading" class="text-center text-stone text-xl">
          <p>Nous rassemblons les plus beaux spécimens pour votre extérieur...</p>
        </div>
        ...
        <div v-else-if="products.length === 0" class="text-center text-stone text-xl">
          <p>Nos bonsaïs d'extérieur se préparent pour la nouvelle saison.</p>
          <p class="mt-4">Revenez bientôt pour découvrir notre collection renouvelée !</p>
        </div>
  ...
  onMounted(async () => {
    try {
      console.log("Chargement des bonsaïs d'extérieur")
      const q = query(collection($db, 'products'), where('category', '==', 'exterieur'))
      const querySnapshot = await getDocs(q)
      console.log("Nombre de bonsaïs d'extérieur trouvés:", querySnapshot.size)
      products.value = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }))
      console.log("Bonsaïs d'extérieur chargés:", products.value)
    } catch (err) {
      console.error("Erreur lors du chargement des bonsaïs d'extérieur:", err)
